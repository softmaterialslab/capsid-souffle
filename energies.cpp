...
void update_LJ_energies_simplified(vector<BEAD>& subunit_bead, double rcut, vector<vector<int> > lj_a){
	 
	double rcut6 = rcut * rcut * rcut * rcut * rcut * rcut;
	double rcut12 = rcut6 * rcut6;
	double shc = 1.2;
		 ...
		 for (unsigned int j = 0; j < subunit_bead.size(); j++){
			 if (subunit_bead[i].itsS[0]->id != subunit_bead[j].itsS[0]->id){
				 ...
				 if ( r < (del+1.12246205*shc) && lj_attractive == false){							//Repulsive
					 ...
					 subunit_bead[i].ne += 0.5*((4 * elj * (shc6 / r6) * ((shc6 / r6) - 1)) + elj);
				 } else if ( r < ((del+1.12246205*shc)*rcut) && lj_attractive == true ){			//Attractive
					 r6 = (r-del) * (r-del) * (r-del) * (r-del) * (r-del) * (r-del);
					 double elj = 2;//subunit_bead[j].epsilon;
					 subunit_bead[i].ne += 0.5* (((4 * elj * (shc6 / r6) * ((shc6 / r6) - 1)) -
															 (4 * elj * ((1 / rcut12) - (1 / rcut6)))));
				 } else {
					 subunit_bead[i].ne += 0;
				 }
			 }
